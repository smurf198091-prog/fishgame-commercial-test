...
function backendEvent(type, payload)
{
	try
	{
		payload = payload || {};
		...
		var send = function(token){
			fetch("/api/event", {method:"POST", headers:{"Content-Type":"application/json", "X-Game-Session":token}, body:body, keepalive:true}).catch(function(){});
		};
		fetch("/api/session").then(function(r){return r.json();}).then(function(s){ if(s && s.token) send(s.token); }).catch(function(){});
	}catch(e){}
}
...
Player.prototype.captureFish = function(fish)
{
	...
	backendEvent("capture", {player_id:this.id || "local-player", fish_id:fish.id, fish_type:fish.type && fish.type.id, coin_value:fish.coin, coin:this.coin, captured:this.numCapturedFishes});
};
